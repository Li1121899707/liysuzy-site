				...
				<div class="tpl-portlet-components">
					<div class="portlet-title">
						<div class="caption font-green bold">
							<span class="am-icon-pencil-square"></span>&nbsp; &nbsp; 管理随想
						</div>
							...
							<form class="am-form">
								<table
									...
									<thead>
										<tr>
											...
											<th class="table-title">随想音符</th>
											<th class="table-main">随想音乐</th>
											<th class="table-main">音乐链接</th>
											<th class="table-main">喜爱</th>
									...
									<tbody>
										...
										<c:if test="${not empty pageUtil.list }">
											<c:forEach items="${pageUtil.list }" var="sr"
												...
												<tr>
													...
													<td>${sr.capNote}</td>
													<td>${sr.capMusic}</td>
													<td><a href="${sr.capLink}" target="_blank">${sr.capLink}</a></td>
													<td><c:if test="${sr.capFavor==1 }">★</c:if> <c:if
															test="${sr.capFavor==2 }">★★</c:if> <c:if
															test="${sr.capFavor==3 }">★★★</c:if> <c:if
															test="${sr.capFavor==4 }">★★★★</c:if> <c:if
															test="${sr.capFavor==5 }">★★★★★</c:if></td>

													<td><a
														href="${pageContext.request.contextPath }/capriccio/${sr.capId}/update?form">
								...
								<div class="am-cf">
									...
									<div class="am-fr">
										<ul class="am-pagination tpl-pagination">
											<li><a
												href="${pageContext.request.contextPath }/capriccio/list?pageIndex=1">首页</a>
											</li>
											<c:choose>
												<c:when test="${pageUtil.pageIndex - 1 > 0}">
													<li><a
														href="${pageContext.request.contextPath }/capriccio/list?pageIndex=${pageUtil.pageIndex - 1}">上一页</a>
													</li>
												</c:when>
											</c:choose>
											...
											<c:forEach begin="1" end="${pageUtil.pageCount}" var="i">
												<li><a
													href="${pageContext.request.contextPath }/capriccio/list?pageIndex=${i}"
											...
											<c:choose>
												<c:when test="${pageUtil.pageCount==0}">
													<li><a
														href="${pageContext.request.contextPath }/capriccio/list?pageIndex=${pageUtil.pageIndex }">下一页</a>
													</li>
												</c:when>
												<c:when
													test="${pageUtil.pageIndex + 1 <= pageUtil.pageCount}">
													<li><a
														href="${pageContext.request.contextPath }/capriccio/list?pageIndex=${pageUtil.pageIndex+1}">下一页</a>
													</li>
												</c:when>
											...
											<c:choose>
												<c:when test="${page.pageCount==0}">
													<li><a
														href="${pageContext.request.contextPath }/capriccio/list?pageIndex=${pageUtil.pageIndex}">尾页</a>
													</li>
												</c:when>
												<c:otherwise>
													<li><a
														href="${pageContext.request.contextPath }/capriccio/list?pageIndex=${pageUtil.pageCount}">尾页</a>
													</li>
